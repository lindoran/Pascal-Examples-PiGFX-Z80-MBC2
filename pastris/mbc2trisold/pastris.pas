...
const
 ...
 spdoffset = 1.6;   { slows game down for keyboard only users, this
                      is needed to make the keyboard perform at the
                      same speed as the joystick, due to the added
                      operations that the joystick requires
                     }

 difstart = 400; { starting difficulty in number of loops before play
                    advances, this is very inacurate way of tracking
                    time in a game but simple }
 difoffset = 39; { this is the offset each difficulty level dificulty is
                    a real number but must be a positive result or it
                    may cause issues }
...
var
 ...
 T1,L,lev,S,E,P,PC,PP,ROT,X,Y,XT,YT,YY,CY,ROW : integer;
 temp,failout,sndbit,joybit,startloop : boolean;
 T2,timer : real;

 { drop a letter input widget any place on the screen and output a
   string of initials how usefull! lol will probably integrate this
   into the next version of the graphics unit }
...
{define the squares that make up the pieces and the blanking tiles
 sends bitmaps in RLE to the graphics interface starting at index 0
 this works well because the color numbers in the source refrence the
 same index number for color when useing the maximite palette.  Im not
 actually useing hardware sprites because the orginal program did not
 and the interface runs fast enough they were not needed}
...
begin
 for P := 0 to 7 do
  begin
   color := NumStr(P);
   dot := ';7;0;1;';   {this is the hex code that programs the RLE pixels}
...
begin
 ...
 for dd := 1 to 10 do
  begin
   if joybit = true then level[dd] := difficulty else level[dd] := difficulty * spdoffset;
 ...
 if sndbit = true then bprint(15,130,'(*)');
 if joybit = true then bprint(15,140,'(*)');
...
begin
  ...
  if joybit = true then if getjoy > 0 then
   begin
     userin := getjoy;
     if userin > 16 then delay(dbdelayfire) else delay(dbdelaydirec);
   end;
   case inputtbl[userin] of
        1 : moveleft;
        2 : moveright;
